...
def test_assign_role(app, client, test_users, test_roles):
    ...
    with app.app_context():
        ...
        usr.assign_role("test_role")
        assert usr.has_role("test_role")
...
def test_deassign_role(app, client, test_users, test_roles):
    ...
    with app.app_context():
        ...
        usr.assign_role("test_role")
        usr.assign_role("test_role1")

        assert usr.has_role("test_role")
        assert usr.has_role("test_role1")

        usr.deassign_role("test_role")
        assert not usr.has_role("test_role")
        assert usr.has_role("test_role1")
...
def test_decorators(app, client, test_users, test_roles, test_admin):
    """Testing roles_accepted and role_required decorators"""

    @app.route("/accepted_basic_roles")
    @roles_accepted("test_role1", "test_role")
    def test1():
        return "OK"
